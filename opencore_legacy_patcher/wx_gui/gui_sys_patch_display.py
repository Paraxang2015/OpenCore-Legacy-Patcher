...
class SysPatchDisplayFrame(wx.Frame):
    ...
    def __init__(self, parent: wx.Frame, title: str, global_constants: constants.Constants, screen_location: tuple = None):
        ...
        if self.constants.update_stage != gui_support.AutoUpdateStages.INACTIVE:
            if self.available_patches is False:
                gui_support.RestartHost(self.frame).restart(
    ...
    def _generate_elements_display_patches(self, frame: wx.Frame = None) -> None:
        ...
        available_label = wx.StaticText(
            frame,
            label="Fetching patches for host",
            pos=(-1, title_label.GetPosition()[1] + title_label.GetSize()[1] + 10)
        )
        ...
        progress_bar = wx.Gauge(
            frame,
            range=100,
            pos=(-1, available_label.GetPosition()[1] + available_label.GetSize()[1] + 10),
            size=(250, 20)
        )
        ...
        # 🔒 SAFE ACCESS (FIX)
        can_unpatch: bool = not patches.get(
            HardwarePatchsetValidation.UNPATCHING_NOT_POSSIBLE,
            False
        )

        if not any(
            not patch.startswith("Settings")
        ...
        if not patches:
            patch_label = wx.StaticText(
                frame,
                label="No patches required",
                pos=(-1, available_label.GetPosition()[1] + 20)
            )
            ...
            if no_new_patches is True:
                patch_label = wx.StaticText(
                    frame,
                    label="All applicable patches already installed",
                    pos=(-1, available_label.GetPosition()[1] + 20)
                )
                patch_label.SetFont(gui_support.font_factory(13, wx.FONTWEIGHT_NORMAL))
                patch_label.Centre(wx.HORIZONTAL)
                ...
                longest_patch = ""
                for patch in patches:
                    if not patch.startswith(("Settings", "Validation")) and patches[patch] is True:
                        longest_patch = max(longest_patch, patch, key=len)

                anchor = wx.StaticText(
                    frame,
                    label=longest_patch,
                    pos=(-1, available_label.GetPosition()[1] + 20)
                )
                ...
                for patch in patches:
                    if not patch.startswith(("Settings", "Validation")) and patches[patch] is True:
                        ...
                        patch_label = wx.StaticText(
                            frame,
                            label=f"- {patch}",
                            pos=(anchor.GetPosition()[0], available_label.GetPosition()[1] + i)
                        )
                        patch_label.SetFont(gui_support.font_factory(13, wx.FONTWEIGHT_NORMAL))

            if patches.get(HardwarePatchsetValidation.PATCHING_NOT_POSSIBLE, False) is True:
                patch_label = wx.StaticText(
                    frame,
                    label="Cannot patch due to the following reasons:",
                    pos=(-1, patch_label.GetPosition()[1] + 25)
                )
                patch_label.SetFont(gui_support.font_factory(13, wx.FONTWEIGHT_BOLD))
                patch_label.Centre(wx.HORIZONTAL)

        start_button = wx.Button(
            frame,
            label="Start Root Patching",
            pos=(10, patch_label.GetPosition().y + 25),
            size=(170, 30)
        )
        start_button.Bind(wx.EVT_BUTTON, lambda event: self.on_start_root_patching(patches))
        start_button.Centre(wx.HORIZONTAL)

        revert_button = wx.Button(
            frame,
            label="Revert Root Patches",
            pos=(10, start_button.GetPosition().y + 25),
            size=(170, 30)
        )
        ...
        return_button = wx.Button(
            ...
            pos=(10, revert_button.GetPosition().y + 35),
            size=(150, 30)
        )
        return_button.Bind(
            wx.EVT_BUTTON,
        ...
        if not patches:
            ...
            if patches.get(HardwarePatchsetValidation.PATCHING_NOT_POSSIBLE, False):
                start_button.Disable()

        if not can_unpatch:
            revert_button.Disable()

        frame.SetSize((-1, return_button.GetPosition().y + 60))
        frame.ShowWindowModal()
    ...
    def on_return_to_main_menu(self, event: wx.Event = None):
        frame_modal = event.GetEventObject().GetParent()
        frame = frame_modal.Parent
        frame_modal.Destroy()
        frame.Hide()
